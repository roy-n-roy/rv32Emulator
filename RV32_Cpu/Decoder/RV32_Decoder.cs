...
namespace RiscVCpu.Decoder {
    ...
    public class RV32_Decoder {
        ...
        /// </summary>
        /// <param name="instraction">32bit長の命令</param>
        /// <param name="cpu">命令を実行するRV32CPU</param>
        public void Decode(UInt32 instraction, RV32_Cpu cpu) {
            foreach (RV32_AbstractDecoder d in decoders.Values) {
                d.Exec(instraction, cpu);
            }
        }
    }
    ...
    public abstract class RV32_AbstractDecoder {
        ...
        internal protected abstract bool Exec(uint instruction, RV32_Cpu cpu);
        ...
        /// <returns></returns>
        private protected byte[] SplitInstruction(UInt32 instruction) {
        ...
        private protected Int32 GetImmediate(string format, byte[] ins) {
            int result;
            switch (format) {
                case "U":
                    result = ins[2] + (ins[3] << 3) + (ins[4] << 8) + (ins[5] << 13);
                    result += ins[6] == 0 ? 0 : -(1 << 19);
                    break;
                case "J":
                    result = (ins[4] + (ins[5] << 5) + (ins[2] << 11) + (ins[3] << 14));
